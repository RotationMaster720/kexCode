...
namespace ex1 {
    ...
    std::string problem_name = "channel-ex1";
...
int main(int argc, char **argv) {
    ...
    for (int j = 0; j < mesh_refinements; ++j) {
        ...
        while (iter < num_time_steps) {
            ...
            if (iter == 0) {
                interpolate(Vh, data_uh0, uexact_initial);
            } else {
            ...
            while (1) {
                ...
                navier_stokes.addLinear(
                    + innerProduct(u_exact.exprList(), nu*rho*grad(v)*n) 
                    - innerProduct(u_exact.exprList(), boundary_penalty*nu*rho/h*v)
                    - innerProduct(u_exact*n, boundary_penalty/h*v*n)
                    + innerProduct(u_exact*n, q)
                    , surface
                    , In);
