...
namespace ex1 {
    ...
    std::string problem_name = "channel-5_3";
    ...
    double bottom_left_y = -0.5; // Bottom left corner y coordinate
    double width_domain  = 3; // Width of the domain
    double height_domain = 1; // Height of the domain

    const double T = 40; // 40
    const double um = 1;

    double radius = 0.2;
    double center_x = 1.545;
    double center_y = 0;
    double A = 0.8;
    double omega = 0.2;
    ...
    double fun_levelset(double* P, int i, const double t) {
        ...
        return -(((x - center_x - A*std::sin(omega*t)) * (x - center_x - A*std::sin(omega*t)) + (y - center_y) * (y - center_y)) - radius*radius);
    }
    ...
    double surface_fun(double* P, int component, double t) {
        double x = P[0];
        double y = P[1];

        if (component == 0)
            return std::pow(height_domain/2,2) - std::pow(y,2);
            //return std::cos(y*M_PI)*std::sin(t)*std::sin(x*M_PI)*std::sin(x*M_PI)*std::sin(y*M_PI);
        else
            return 0;
...
int main(int argc, char **argv) {
    ...
    for (int j = 0; j < mesh_refinements; ++j) {
        ...
        dT = T / num_time_steps; // här skrivs dt över
        ...
        while (iter < num_time_steps) {
            ...
            while (1) {
                ...
                if (newton_iterations == 0) {
                    ...
                    // Set averages of pressures (and u*n)
                    // navier_stokes.addBilinear(
                    //     + innerProduct(xi_u, v*n)
                    //     + innerProduct(u*n, chi_u)
                    //     , surface
                    //     , In
                    // );
                    // navier_stokes.addBilinear(
                ...
                navier_stokes.addLinear(
                    ...
                    , INTEGRAL_BOUNDARY
                    , In
                    , inflow_and_walls); // inflow_and_walls
                    
                navier_stokes.addLinear(
                    + innerProduct(body.exprList(), nu*rho*grad(v)*n) 
                    - innerProduct(body.exprList(), boundary_penalty*nu*rho/h*v)
                    - innerProduct(body*n, boundary_penalty/h*v*n)
                    + innerProduct(body*n, q)
                    , surface
                    , In);
            ...
            // double area   = integral(active_mesh, 1.);
            // // double int_ph = integral(active_mesh, In, ph.expr(), q_time, last_quad_pt_time, 0);  // gives different (incorrect) results
            // double int_ph = integral(active_mesh, ph_T.expr());
            // // double int_p  = integral(active_mesh, In, p_exact.expr(), q_time, last_quad_pt_time, 0);
            // double int_p  = integral(active_mesh, p_exact, 0, last_quad_pt_time);
            // double average  = (int_p - int_ph) / area; 
            // std::transform(ph_T.v.begin(), ph_T.v.end(), ph_T.v.begin(),
            ...
            // double area                = integral(active_mesh, 1.);
            // double int_div             = integral(active_mesh, uh_0dx + uh_1dy);
            // double intf_cond           = integral(uh_T*n, surface);
            // double intf_cond_exact     = integral(u_exact*n, surface);

            // std::cout << " --- DATA: --- " << '\n';
            // std::cout << " int_Omega div(uh) = " << int_div << '\n';
            // std::cout << " int_Gamma uh*n = " << intf_cond << '\n';
            // std::cout << " int_Gamma g*n = " << intf_cond_exact << '\n';
            // std::cout << "\n";
            ...
            if (MPIcf::IamMaster() && (mesh_refinements == 1)) {
                ...
                // Paraview<mesh_t> paraview_surf(active_mesh_surface, path_output_figures + "stokes_lagr_surf_" + std::to_string(j) + ".vtk");
                // paraview_surf.add(xih, "lagrange", 0, 2);
                // paraview_surf.add(xi_exact, "lagrange_exact", 0, 2);
                // paraview_surf.add(xi_error, "lagrange_error", 0, 2);
            }
